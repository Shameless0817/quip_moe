...
def forward_layer_qwen15moe_experts(
    layer,
    position_ids,
    attention_mask,
    bs,
    ...
    """Forward pass for Qwen1.5-MoE layer - collecting Expert Hessians (gate + experts for each expert)."""
    layer = layer.to(device)
    position_ids = position_ids.to(device)
    attention_mask = attention_mask.to(device)

    # Qwen1.5-MoE uses 'mlp' module
    ...
    for expert_idx in range(num_experts):
        ...
        # Qwen1.5-MoE experts have: gate_proj, up_proj, down_proj
    ...
    # Qwen1.5-MoE might have shared_expert_gate instead of a full shared expert module
    if has_shared_expert:
        ...
        elif hasattr(moe_module, 'shared_expert_gate'):
            # Some versions might have shared_expert_gate as a separate parameter
            done_experts['shared_expert_gate'] = utils.register_H_hook(moe_module.shared_expert_gate, device)
            print(f"Including shared expert gate")
...
def count_linear_layers_qwen15moe_experts(layer, num_experts, has_shared_expert=False):
    """Count expected linear layers in Qwen1.5-MoE experts."""
    # gate: 1 layer
    ...
    if has_shared_expert:
        moe_module = layer.mlp
        if hasattr(moe_module, 'shared_expert'):
            # Full shared expert: gate_proj, up_proj, down_proj: 3 layers
            count += 3
        elif hasattr(moe_module, 'shared_expert_gate'):
            # Just shared expert gate: 1 layer
            count += 1
...
def detect_qwen15moe_config(model):
    ...
    # Check which layers are MoE layers
...
def main(args):
    ...
    for transformer_layer_index in range(num_layers):
        ...
        expected_count = count_linear_layers_qwen15moe_experts(
        ...
        assert hasattr(transformer_layer, 'mlp'), "Layer does not have 'mlp' attribute"
        assert hasattr(transformer_layer.mlp, 'gate'), "MLP does not have 'gate' attribute"
        assert hasattr(transformer_layer.mlp, 'experts'), "MLP does not have 'experts' attribute"
        assert len(transformer_layer.mlp.experts) == args.num_experts, \
            f"Expected {args.num_experts} experts, found {len(transformer_layer.mlp.experts)}"

        # Forward pass and collect Hessians
        print("Running forward pass to collect Hessians...")
        results = forward_layer_qwen15moe_experts(
            transformer_layer,
            position_ids,
            attention_mask,
            args.batch_size,
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Collect Hessian matrices for Qwen1.5-MoE expert layers')
