...
class QuantizedE8P12LinearWithLoRA(nn.Module):
    def __init__(self, original_module, lora_rank=16):
        super().__init__()
        
        self.Qidxs_list = original_module.Qidxs_list
        self.scale = original_module.scale
        self.SU = original_module.SU
        self.SV = original_module.SV
        self.had_left = original_module.had_left
        self.had_right = original_module.had_right
        self.K_left = original_module.K_left
        self.K_right = original_module.K_right
        self.bias = original_module.bias
        
        # 获取维度
        self.d_out = original_module.Qidxs_list[0].shape[0] * 12
        self.d_in = original_module.Qidxs_list[0].shape[1] * 8
        
        # 低秩补偿参数
        self.lora_rank = lora_rank
        self.lora_A = None  # [d_in, r]
        self.lora_B = None  # [d_out, r]
        
    def set_lora_matrices(self, A, B):
        """设置低秩补偿矩阵"""
        self.lora_A = nn.Parameter(A, requires_grad=False)
        self.lora_B = nn.Parameter(B, requires_grad=False)
    ...
    def forward(self, x):
        ...
        # Hadamard预处理
        had_left_T = self.had_left.T
        x = x * self.SU
        x = matmul_hadU_cuda(x, had_left_T, self.K_left) / self.scale
        
        # 解压缩量化权重并计算
        W_decompressed = self._decompress_weights()
        x_quantized = (x.to(torch.float16) @ W_decompressed.T).to(torch.float32)
        
        # Hadamard后处理
        x_quantized = matmul_hadU_cuda(x_quantized, self.had_right, self.K_right)
        x_quantized = x_quantized * self.SV * self.scale
        
        # 添加低秩补偿
        if self.lora_A is not None and self.lora_B is not None:
            x_input = x.reshape(-1, self.d_in)
            x_lora = (x_input @ self.lora_A) @ self.lora_B.T
            x_quantized = x_quantized + x_lora
        
        # 添加bias
        if self.bias is not None:
            x_quantized = x_quantized + self.bias
            
        return x_quantized.reshape(*batch_shape, -1)
...
class QuantizedE8P12LinearWithLoRA(nn.Module):
    def __init__(
        ...
        self.lora_rank = rank
        if rank > 0: 
            self.lora_A = nn.Parameter(torch.zeros(self.codebook.codesz * 8, rank).to(device), requires_grad=True)
            self.lora_B = nn.Parameter(torch.zeros(self.codebook.codesz * 8, rank).to(device), requires_grad=True)
            ## 核对一下size
        else: 
            self.lora_A = None
            self.lora_B = None
    ...
    def set_lora_matrices(self, A, B):
        ...
        Args:
            ...
            B: [d_out, r] 输出侧低秩矩阵
        """
        if A.shape[0] != self.d_in:
            raise ValueError(f"A的第一维度 {A.shape[0]} 必须等于 d_in {self.d_in}")
        if B.shape[0] != self.d_out:
            raise ValueError(f"B的第一维度 {B.shape[0]} 必须等于 d_out {self.d_out}")
        if A.shape[1] != B.shape[1]:
            raise ValueError(f"A和B的秩必须相同: {A.shape[1]} vs {B.shape[1]}")
            
        self.lora_A = nn.Parameter(A.to(self.device), requires_grad=False)
        self.lora_B = nn.Parameter(B.to(self.device), requires_grad=False)
    ...
    def forward(
        ...
        **kwargs
        ...
        x = x * SV * self.scale 
        output_quantized = x.view(*input.shape[:-1], m)

        # 添加低秩补偿
        if self.lora_A is not None and self.lora_B is not None:
            x_lora = (x_input @ self.lora_A) @ self.lora_B.T 
            x_lora = x_lora.view(*input.shape[:-1], m) 
            output = output_quantized + x_lora
        else:
