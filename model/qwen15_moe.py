...
#
# This code is based on the Qwen1.5 model implementation with quantization
...
"""PyTorch Qwen1.5 MoE model with quantization support."""
...
class Qwen2MoeLinearScalingRotaryEmbedding(Qwen2MoeRotaryEmbedding):
    """Qwen2MoeRotaryEmbedding extended with linear scaling."""
...
class Qwen2MoeDynamicNTKScalingRotaryEmbedding(Qwen2MoeRotaryEmbedding):
    """Qwen2MoeRotaryEmbedding extended with Dynamic NTK scaling."""
...
# =============================================================================
# Quantized Expert MLP (Qwen1.5 style with shared expert support)
# =============================================================================

class Qwen2MoeQuantizedExpertMLP(nn.Module):
    """
    A single expert MLP with quantized linear layers (Qwen1.5 style).
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        # Qwen style: down_proj(act(gate_proj(x)) * up_proj(x))
        return self.down_proj(self.act_fn(self.gate_proj(x)) * self.up_proj(x))


# =============================================================================
# Shared Expert MLP (for Qwen1.5 MoE)
# =============================================================================

class Qwen2MoeSharedExpertMLP(nn.Module):
    """
    Shared expert MLP used in Qwen1.5 MoE models.
    This expert is always activated for all tokens.
    """
...
# =============================================================================
# Sparse Mixture of Experts Layer (Qwen1.5 style with shared expert)
# =============================================================================

class Qwen2MoeSparseMoeBlock(nn.Module):
    """
    Sparse Mixture of Experts block with quantized expert MLPs (Qwen1.5 style).
    
    Includes:
    - Multiple routed experts
    - One shared expert (always activated)
    - Top-k routing with normalization
    """
    def __init__(self, config: Qwen2MoeConfig):
        super().__init__()
        self.config = config
        ...
        # Qwen1.5 specific: norm before gating
    ...
    def forward(self, hidden_states: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        """
        Forward pass for the Qwen1.5 MoE layer.
        ...
        # Normalize top-k weights (Qwen1.5 specific)
        ...
        # Add shared expert output (Qwen1.5 specific)
...
# =============================================================================
# Attention Modules (Qwen1.5 style)
# =============================================================================

class Qwen2MoeAttention(nn.Module):
    """
    Multi-headed attention with quantized projections for Qwen1.5.
    ...
    def _init_rope(self):
        ...
        if rope_scaling is None:
            ...
            if scaling_type == "linear":
                self.rotary_emb = Qwen2MoeLinearScalingRotaryEmbedding(
                    self.head_dim,
                    max_position_embeddings=self.max_position_embeddings,
                ...
                self.rotary_emb = Qwen2MoeDynamicNTKScalingRotaryEmbedding(
                    self.head_dim,
                    max_position_embeddings=self.max_position_embeddings,
                    scaling_factor=scaling_factor,
                    base=self.rope_theta,
...
class Qwen2MoeFlashAttention2(Qwen2MoeAttention):
    """
    Qwen1.5 flash attention module using Flash Attention 2.
    """
...
class Qwen2MoeSdpaAttention(Qwen2MoeAttention):
    """
    Qwen1.5 attention module using torch.nn.functional.scaled_dot_product_attention.
    """
...
# =============================================================================
# Decoder Layer (Qwen1.5 style)
# =============================================================================

class Qwen2MoeDecoderLayer(nn.Module):
    """
    Single transformer decoder layer with MoE (Qwen1.5 style).
    """
...
@add_start_docstrings(
    "The bare Qwen1.5 MoE Model outputting raw hidden-states without any specific head on top.",
...
@add_start_docstrings(
    """
    The Qwen1.5 MoE Model transformer with a sequence classification head on top (linear layer).
