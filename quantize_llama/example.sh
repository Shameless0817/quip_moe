...
# CUDA_VISIBLE_DEVICES=3 python -m quantize_llama.quantize_finetune_llama --save_path $CKPT/2_7b_2bit --codebook E8P12  --scale_override 0.9 --base_model NousResearch/Llama-2-7b-hf  --hessian_path /fact_home/zeyuli/quip_sharp/llama_hessian --devset_size 384 --ft_valid_size 128

# convert model to hf format for end to end fine tuning
CUDA_VISIBLE_DEVICES=3 python -m quantize_llama.hfize_llama --quantized_path $CKPT/2_7b_2bit --hf_output_path $HF/2_7b_2bit
